...
synopsis () {
	...
	echo -e "\tCe nombre doit être compris entre 1 et 3 (par défaut, 2)."
	exit "$1"
}
...
while [[ -n "$1" ]]
	...
	-s|--nombre-semaine)
		if [[ "$2" != -*  && "$2" != "" ]] ; then
			nb_semaines="$2"
			if [[ "$nb_semaines" != [123] || "$nb_semaines" != +([[:digit:]]) ]] ; then
				synopsis 2
			fi
		else
			synopsis 2
...
numero=1
###ft : Création rep temporaire pour fichier informatif :
rep_info=$(mktemp -d)
mkdir -p $rep_info/$promotion
#########################################################
...
do
	...
	then
		...
		tsig-keygen -a hmac-sha512 $utilisateur > /etc/bind/key/$utilisateur.key
		cat > "/etc/bind/zone.$DOMAINE.conf.d/$utilisateur.conf" 2>> $erreurs << FIN
include "/etc/bind/key/$utilisateur.key";
zone "$utilisateur.$DOMAINE" {
	type master;
	file "$utilisateur";
        allow-update { key "$utilisateur"; };
        allow-transfer { key "$utilisateur"; };
};
FIN
#### ft : MODIF creation FICHIER PERSO nommé : $utilisateur
		cat "$zonepardefaut" > "$fichierzone" 2>> "$erreurs"
		chown "$utilisateur":eni "$fichierzone" 2>> "$erreurs"
		chmod 660 "$fichierzone" 2>> "$erreurs"
		cat > "$rep_info/$promotion/$utilisateur.txt" << FIN
...
* Éditez votre zone DNS dans le fichier
	$fichierzone
* Configuration TSIG pour mise à jour automatique :
	serveur DNS :	$SERVER_IP
	TSIG key name :	$utilisateur
	TSIG secret :	$(awk -F'"' '/secret "/ {print $2}' /etc/bind/key/$utilisateur.key)
	Pour éditer votre zone dynamique utilisez la commande "sudo /srv/edit.sh"
...
### FT : création de l'archive
cd $rep_info
zip /home/formateur/$promotion.zip $promotion/* > /dev/null
chown "$(logname):" "/home/formateur/$promotion.zip"
cd - >/dev/null
###########################################################################
...
{
	cat "$zonepardefaut"
	echo "revealjs	A	$SERVER_IP"
	echo "ns	A	$SERVER_IP"

	# zone tssr nécessaire pour la version de FLN du cours services réseaux en environnement Linux
	echo "deb-s2	A	192.168.222.12"
	echo "tssr	NS	deb-s2"
} > "$USERS_HOME/$DOMAINE"

echo -e "\n"
###cat "$affichage"
#######################" ft : remplacement vers un message scp archive.zip 
echo -e "=> exécutez sur votre poste formateur dans une console powershell la commande suivant pour récupérer l'archive des information utilisateur via : 
scp formateur@lab-eni.fr:/home/formateur/$promotion.zip .
/!\\ attention le point : \".\" est important /!\\ "
############################################################################
rm "$zonepardefaut"
